...
abstract class Rest_lib extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        $this->load->helper(array('app', 'jwt', 'auth'));
    ...
    public function response($status = NULL, $message = NULL, $data = NULL, $http_code = NULL)
    {
        ob_start();
        if ($http_code !== NULL) {
        ...
        elseif ($status === true && $data === null) {
            $this->output->set_content_type(
                $this->_supported_formats[$this->response->format], strtolower($this->config->item('charset')));
            $data = array(
                'status' => $status,
                'message' => $message
            );
        ...
        elseif ($data !== NULL) {
            if (method_exists($this->format, 'to_' . $this->response->format)) {
                ...
                $response['data'] = $data;
                $output = $this->format->factory($response)->{'to_' . $this->response->format}();
    ...
    public function page_response($status = NULL, $message = NULL, $page = NULL, $total_page = NULL, $total_data = NULL, $data = NULL, $http_code = NULL)
    {
        ob_start();
        if ($http_code !== NULL) {
            $http_code = (int)$http_code;
