...
(function(factory) {
    ...
    var IconLayers = L.Control.extend({
        ...
        _switchMapLayers: function() {
            ...
            if (activeLayer && activeLayer.layer != null) {
                this._map.addLayer(activeLayer.layer);
            }
        },
        options: {
            position: 'topleft', // one of expanding directions depends on this
            behavior: 'previous', // may be 'previous', 'expanded' or 'first'
            expand: 'vertical', // or 'vertical'
        ...
        expand: function() {
            this._arrangeLayers().slice(1).map(function(l) {
                let el = this._getLayerCellByLayerId(l.id);
                L.DomUtil.removeClass(el, 'leaflet-iconLayers-layerCell_hidden');
            }.bind(this));
        },
        collapse: function() {
            this._arrangeLayers().slice(3).map(function(l) {
                let el = this._getLayerCellByLayerId(l.id);
    ...
    let iconLayers = function(layers, options) {
